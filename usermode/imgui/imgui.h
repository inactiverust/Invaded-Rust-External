...
namespace ImGui
{
    ...
    IMGUI_API bool          HotKey(const char* label, int* k, const ImVec2& size_arg);// allow last item to be overlapped by a subsequent item. sometimes useful with invisible buttons, selectables, etc. to catch unused area.
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText(const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, float font_size = 0.0f);
